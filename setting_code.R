#### Returning Values:
# lambda0, p, q, r, OmegaT, sigmaOmegaT, betaT, sigmabetaT, Time, tau, N
#### Hypothetical MSM Trial
### Mimicing HPTN083
SettingMSM_para <- function(q, r, tau, N=NULL){
	...
	FRR <- c(0.0032,0.0097,0.015,0.0385,NA)
	...
	return(list(lambda0=pop_inc, p=pop_p, q=q, r=r, OmegaT=pop_MDRI, sigmaOmegaT=pop_MDRI_sigma, betaT=pop_FRR, sigmabetaT=sigmaFRR, Time=Time, tau=tau, N=N))
}
#### Hypothetical Women Trial
### Mimicing HPTN084
SettingWomen_para <- function(q, r, tau, N=NULL){
	...
	return(list(lambda0=lambda0, p=p, q=q, r=r, OmegaT=MDRI, sigmaOmegaT=sigmaMDRI, betaT=FRR, sigmabetaT=sigmaFRR, Time=Time, tau=tau, N=N))
}
