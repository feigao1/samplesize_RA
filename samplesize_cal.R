...
tau_set = c(1,2); R0 = 0.5; R1 = 0.15; alpha=0.05; beta=0.9
#### MSM ####
for (tau in tau_set){
	para = SettingMSM_para(q=1, r=0.85, tau=tau, N=NULL)
	N  = samplesize_CF(para, R1, R0, alpha,beta)
...
tau_set = c(1,2); R0 = 0.5; R1 = 0.15; alpha=0.05; beta=0.9
for (tau in tau_set){
	para = SettingWomen_para(q=1, r=0.85, tau=tau, N=NULL)
	N  = samplesize_CF(para, R1, R0, alpha, beta)
	N_crit = summarize_expectnum(N,para,R1) # N, N+, Nrec, N-enroll, Nevent
	print(c(tau,N_crit))
}
